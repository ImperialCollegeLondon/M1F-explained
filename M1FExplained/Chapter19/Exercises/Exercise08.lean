...
theorem dvd_pow_sub_pow (a b : R) (n : ℕ) : a - b ∣ a ^ n - b ^ n :=
  ...
  rw [Finset.sum_range_succ']
  suffices
    ∑ x : ℕ in Finset.range n, a * (a ^ x * b ^ (n - x)) =
      ∑ k : ℕ in Finset.range n, b * (a ^ (k + 1) * b ^ (n - (k + 1)))
    by rw [this, Nat.succ_eq_add_one, Nat.sub_self]; ring
...
example :
  ...
  have h1 : ∃ k1 : ℤ, b - c = k1 * (a - b) := by specialize h a b; rwa [hP1, hP2] at h ;
    exact exists_eq_mul_left_of_dvd h
  have h2 : ∃ k2 : ℤ, c - a = k2 * (b - c) := by specialize h b c; rwa [hP2, hP3] at h ;
    exact exists_eq_mul_left_of_dvd h
  have h3 : ∃ k3 : ℤ, a - b = k3 * (c - a) := by specialize h c a; rwa [hP3, hP1] at h ;
    exact exists_eq_mul_left_of_dvd h
